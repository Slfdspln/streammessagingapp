  ...
  return (
    ...
    >
      ...
      <Avatar 
        ...
        name={channel.data.name || Object.values(channel.state.members).map(m => m.user.name).join(', ')} 
        size={60} 
      />
      <View style={styles.textContainer}>
        <Text style={styles.title} numberOfLines={1}>
          {/* If the channel has a name, use it, otherwise show member names */}
          {channel.data.name || Object.values(channel.state.members).map(m => m.user.name).join(', ')}
        </Text>
